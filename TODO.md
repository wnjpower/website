...
### 헤더·CTA 개선 (2026-06-14 2차)
- [x] **서비스 순서 변경** — 전기공사 → 실내 인테리어 전기 → 분전함 자체 제작
- [x] **견적 폼 고객유형 세분화** — 3단계 구조(고객유형 선택 → 공사유형 → 연락처), 주택·카페상가·상업건물 고객군별 카테고리 동적 필터링
- [x] **카카오톡 공식 브랜드 적용** — `#FEE500` 배경·`#3C1E1E` 아이콘 커스텀 SVG (`KakaoIcon.tsx`)
- [x] **"출장비 무료" 카피 명확화** — "현장 견적 출장비 무료"로 전 영역 통일 (FAQ 답변 포함)
- [x] **상단 공지 띠 개편** — 영업시간·이메일·현장 견적 출장비 무료 3항목만 구성, 높이 40px → 56px, 정적 분할 배치 (얇은 구분선)
- [x] **헤더 전화 버튼 색상 변경** — `#FFB800` 옐로 → `#FF5500` 오렌지 + 흰 텍스트 (데스크톱·모바일 동일)

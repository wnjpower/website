...
- [ ] **카카오톡 채널 URL / 네이버 플레이스·블로그 URL** — [`lib/site.ts`](lib/site.ts)
      한 곳만 입력하면 Contact·SchemaOrg에 자동 반영(현재 `null`이라 버튼 숨김)
...
- [ ] **영업시간 확정** — 현재 `평일 09:00–18:00 · 토 09:00–13:00`.
      확인 후 [`components/SchemaOrg.tsx`](components/SchemaOrg.tsx)의 `openingHours`도 일치시킬 것

## 🟢 SEO · 분석 (운영 등록)

- [ ] **🔴 네이버 스마트플레이스 등록·최적화** — 보고서가 꼽은 **네이버 유입 최우선**.
      로컬 검색 최상단이 플레이스 영역인데 현재 미등록.
      미클레임 플레이스 확인 → 없으면 신규 등록(90일 부스팅)

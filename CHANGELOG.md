...
## 2026-08-09 — AEO 결과 CSV 내보내기 (구글 시트 연동)

AEO 추적 결과를 CSV로 내려받아 구글 시트에서 관리할 수 있게 했다.
`/api/admin/aeo/export?type=keywords|observations` (관리자 전용).

### 두 파일로 나눈 이유

성격이 다르다. **진단은 스냅샷**(오늘 기준 어떤 상태인가)이고 **관찰 기록은
시계열**(언제 무엇을 봤는가)이다. 한 표에 섞으면 시트에서 둘 다 다루기 어렵다.

진단 CSV의 첫 칸이 `기준일`인 것도 같은 이유다. 같은 시트에 계속 덧붙이면
날짜별 점수 변화를 피벗으로 볼 수 있다.

| 파일 | 칸 |
|---|---|
| 키워드 진단표 | 25칸 — 기준일·키워드·질문·점수·등급·담당 페이지·인용후보문장 + `통계:` 5칸 + `진단:` 8칸 + 메모 |
| 관찰 기록 | 7칸 — 관찰일시·키워드·질문·답변엔진·인용여부·인용된대목·메모 |

`통계:` / `진단:` 접두어를 붙였다. 접두어가 없으면 통계의 「질문어적중」(숫자)과
판정의 「질문어 적중」(양호/보통/미흡)이 **띄어쓰기 하나로만** 구분돼, 시트에서
두 칸이 나란히 보이면 어느 쪽이 무엇인지 알 수 없다.

### 한국어 CSV가 깨지는 세 가지를 다 막았다 — [`lib/csv.ts`](lib/csv.ts)

이 저장소의 첫 CSV 기능이라 유틸을 따로 두었다.

1. **글자 깨짐** — 엑셀은 BOM이 없으면 UTF-8 CSV를 CP949로 읽어 전부 `????`가
   된다. BOM(`EF BB BF`)을 붙인다. 구글 시트는 없어도 되지만 있어도 무해하다.
2. **칸 밀림** — 값 안의 쉼표·따옴표·줄바꿈을 RFC 4180대로 처리(따옴표로 감싸고
   내부 따옴표는 두 번). 줄바꿈은 CRLF.
3. **수식 주입(CSV injection)** — 시트는 `=`, `+`, `-`, `@`, 탭, CR로 시작하는
   칸을 **수식으로 실행한다.** 「인용된 대목」·「메모」는 답변엔진의 답을 그대로
   붙여넣는 값이라 우리가 통제하지 않는 글이다. `=IMPORTXML(...)` 같은 것이
   남의 스프레드시트에서 실행돼 데이터가 샐 수 있다. 앞에 작은따옴표를 붙여 막았다
   (시트 화면에는 보이지 않는다).

파일 이름에 한글이 들어가므로 `Content-Disposition`은 RFC 5987의 `filename*`
(UTF-8 퍼센트 인코딩)과 ASCII 대체 이름을 함께 보낸다.

### 구글 시트 `IMPORTDATA`는 쓸 수 없다

화면 안내에 명시했다. 관리자 로그인이 필요한데 시트는 쿠키를 보내지 못하고,
인증을 풀면 **어떤 키워드를 노리는지가 공개**된다. 내려받아 올리는 것이 맞다.

### 검증

`tsc` · `lint` · `build` 통과. 라우트 172 B, 공개 사이트 First Load JS 103 kB 변동 없음.

- **인증** — 비로그인 요청은 `401 {"error":"unauthorized"}`
- **전송 바이트** — 실제 응답의 첫 3바이트가 `EF BB BF`임을 확인
  (브라우저 `fetch().text()`는 BOM을 떼므로 `arrayBuffer`로 봐야 한다)
- **이스케이프** — DB에 `=HYPERLINK("http://evil…")`, 쉼표+따옴표+줄바꿈,
  `-1234`를 넣고 내보내 전부 차단·감싸짐을 확인 후 삭제
- 화면 — 새 일렉트릭 블루 스킴에서 정상 렌더 확인

---

## 2026-08-09 — AEO 키워드 추적 기능

ChatGPT·Perplexity 같은 **답변엔진이 우리 문장을 인용하는가**를 키워드 단위로
추적하는 어드민 기능(`/admin/aeo`).

### 왜 SEO 화면과 따로 두는가

SEO는 «검색 결과 목록에서 몇 번째인가», AEO는 «답을 만들 때 우리 문장을 뽑아
쓰는가»다. 답변엔진은 페이지를 요약하지 않고 **문장 단위로 인용**하므로,
「좋은 글」이 아니라 「뽑아 쓰기 좋은 문장이 있는 글」이 인용된다. 판정 기준이
달라 기존 `lib/seo/analyze.ts`(글 단위 SEO 점수)로는 답할 수 없다.

### 구성

| 파일 | 역할 |
|---|---|
| [`lib/seo/corpus.ts`](lib/seo/corpus.ts) | 홈·서비스 상세·FAQ·실적·게시글을 페이지 단위 텍스트로 수집 |
| [`lib/seo/aeo.ts`](lib/seo/aeo.ts) | 인용 준비도를 8개 항목으로 판정하는 순수 함수 |
| [`supabase/migrations/20260809_aeo_tracking.sql`](supabase/migrations/20260809_aeo_tracking.sql) | `aeo_keywords` · `aeo_observations` |
| `app/admin/(dashboard)/aeo/page.tsx` · [`components/admin/AeoManager.tsx`](components/admin/AeoManager.tsx) | 화면 |

**진단 점수는 저장하지 않는다.** 사이트 문구를 고치면 준비도가 바로 달라져야
하므로 화면을 열 때마다 다시 계산한다. 반대로 관찰 기록(그날 ChatGPT가 우리를
인용했는가)은 다시 계산할 수 없는 관측이라 DB에 남긴다.

### 판정 항목

담당 페이지 · 질문어 적중 · 인용 후보 문장 · 직답 위치 · **문장 자립성** ·
검증 가능한 수치 · Q&A 대응 · FAQPage 구조화 데이터.

이 중 «문장 자립성»이 AEO 고유 항목이다. 「저희는 직접 시공합니다」는 페이지
안에서는 읽히지만 답변엔진이 뽑아내면 **주체가 사라져** 인용되지 않는다.
주어(상호)나 장소가 문장 안에 있어야 한다.

### 만들면서 잡은 결함 — 주제를 안 다뤄도 80점이 나왔다

첫 구현에서 「태양광 발전소 시공」이 **81점 「인용될 준비가 됐습니다」**로
나왔다. 사이트에 태양광은 한 줄도 없는데, 가장 가까운 페이지(홈)가 잘 쓰인
페이지라는 이유만으로 인용후보·자립성·FAQ 스키마 점수를 전부 가져갔기 때문이다.

두 곳을 고쳤다.

- **주제 커버리지 게이팅** — 키워드가 어디에도 없고 질문어 적중률이 60% 미만이면
  거기서 끝낸다. 이하 항목은 «페이지»의 품질이지 «이 키워드»의 준비도가 아니다
- **인용 후보 기준 강화** — 질문어 하나만 스쳐도 후보로 잡던 것을, 키워드가 통째로
  들어 있거나 질문어 둘 이상이 겹쳐야 후보로 본다 (「대구」 한 낱말 때문에 무관한
  문장이 후보가 됐다)

고친 뒤 같은 키워드가 **0점**으로, 「사이트가 이 주제를 다루지 않습니다」로 나온다.

한국어 질문어 추출도 손봤다. 의문 어미를 조사보다 **먼저** 떼야 한다 —
순서를 바꾸면 「증설하나요」에서 「요」만 떨어져 「증설하나」라는 쓰레기 낱말이 남는다.

### 도구가 바로 찾아낸 것 — 서비스 상세 4곳의 FAQPage 누락

시드한 키워드 6개 중 4개에서 같은 지적이 나왔다.

> ✗ Q&A가 있는데 구조화 데이터로 내보내지 않습니다

`/services/*` 네 페이지는 각각 고유한 FAQ 4개(합 16개)를 화면에 노출하면서
`FaqSchema`를 붙이지 않는다. 현재 `FAQPage`는 `/`와 `/faq` 두 곳뿐이고,
그 둘은 **같은 항목을 중복 노출**하고 있다 — `FaqSchema`의 주석은 중복이
감점이라며 「정본인 /faq에서만 렌더한다」고 적어 두었으나 실제와 다르다.
판단이 필요한 사안이라 이번 커밋에서는 고치지 않고 기록만 남긴다.

### 검증

`tsc` · `lint` · `build` 통과. `/admin/aeo` 8.24 kB / 111 kB,
공개 사이트 First Load JS 103 kB **변동 없음**(어드민 전용 라우트).

- DB — 중복 차단·길이 제약·cascade 삭제를 트랜잭션에서 확인 후 롤백
- 격리 — `anon`은 두 테이블에 SELECT 권한 자체가 없다(내부 운영 정보)
- 쓰기 경로 — 헤드리스 브라우저로 실제 어드민에 로그인해 [관찰 기록] 저장까지
  눌러, 서버 액션 → `requireAdmin` → RLS → insert가 통하고 `created_by`가
  세션에서 채워지는 것까지 확인(검증 기록은 삭제)

---

## 2026-08-06 — 네이버 로컬 검색 대비 (플레이스 등록 준비)

TODO의 🔴 최우선 항목 「네이버 스마트플레이스 미등록」을 진행했다. 등록 자체는
사장님 계정·사업자 인증이 필요해 대행할 수 없으므로, **등록의 효과를 좌우하는
사이트 쪽을 전부 맞춰 두고 등록용 입력값 시트를 만들었다.**

### 발견 — 구조화 데이터에 토요일 영업이 빠져 있었다

사이트는 「평일 09:00–18:00 · **토 09:00–13:00**」을 안내하는데,
`SchemaOrg`의 `openingHoursSpecification`에는 **월~금만** 하드코딩돼 있었다.

로컬 검색은 구조화 데이터의 영업시간으로 「영업 중」을 판정한다. 즉
**토요일에 「대구 전기공사」를 검색한 발주처에게는 닫힌 업체로 보였다는 뜻이다.**
표시 문구는 콘텐츠 기본값에, 구조화 데이터는 컴포넌트에 따로 있어서 생긴 어긋남이다.

### 고친 방식 — 단일 소스

[`lib/site.ts`](lib/site.ts)에 상수를 두고 세 곳이 전부 여기서 파생되게 했다.

| 새 상수 | 파생되는 곳 |
|---|---|
| `BUSINESS_HOURS` | 사이트 표시(`HOURS_TEXT`) · 구조화 데이터 · 등록 시트 |
| `HOURS_NOTE` | 사이트 표시 · 등록 시트 |
| `SERVICE_AREAS` | 사이트 표시(`SERVICE_AREA_TEXT`) · 구조화 데이터 `areaServed` |
| `MAP_URL` | 연락처 「카카오맵 열기」 · 구조화 데이터 `hasMap` |

파생된 세 문자열이 이전 하드코딩 값과 **바이트 단위로 동일**함을 확인했다 —
화면은 그대로고 구조화 데이터만 채워진다.

> 로컬 SEO에서 NAP(이름·주소·전화)와 영업시간은 **사이트·구조화 데이터·플레이스
> 세 곳이 글자 단위로 같아야** 검색엔진이 동일 업체로 묶는다. 값이 여러 곳에
> 흩어져 있으면 어긋나는 것은 시간 문제다.

### 구조화 데이터 강화

- **`areaServed` 2개 → 7개** — `['대구광역시','경상북도']`처럼 광역으로 뭉뚱그리면
  「경산 전기공사」같은 시군 단위 검색에 걸리지 않는다. 실제 시공 시군을 `City`로 나열
- **`hasMap` 추가** — 길찾기 링크
- **`aggregateRating`은 넣지 않았다** — 실제 리뷰 없이 별점을 넣으면 구글 구조화
  데이터 정책 위반이고, 적발 시 리치 결과 자격이 통째로 박탈된다. 이유를 주석에 남겼다

### 수정 — TODO의 서술이 사실과 달랐다

TODO는 외부 채널 URL에 대해 «한 곳만 입력하면 **Contact**·SchemaOrg에 자동 반영»
이라고 적어 뒀는데, 실제로는 `SAME_AS`를 통해 **`SchemaOrg`의 `sameAs`에만** 연결돼
있었다. URL을 넣어도 연락처에는 아무 버튼도 생기지 않았다.

연락처 섹션에 `ExternalChannels`를 실제로 구현했다. 네이버 플레이스·카카오톡·
네이버 블로그 중 URL이 있는 것만 버튼으로 나오고, 전부 `null`이면 블록 자체가
렌더되지 않는다. 클릭은 `contact_place`·`contact_kakao` 슬롯으로 집계된다.

> 플레이스 링크를 사이트에 거는 것은 편의 기능이 아니다. 사이트 → 플레이스와
> 플레이스 → 사이트가 서로를 가리켜야 「같은 업체」 신호가 완성된다.

### Google Search Console 소유확인 슬롯

네이버 서치어드바이저 확인 코드는 이미 박혀 있는데 구글은 없었다.
`NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` 환경변수를 넣고 재배포하면 meta가 자동으로
붙는다(값이 없으면 키 자체를 넣지 않는다 — 빈 문자열은 소유확인을 오히려 실패시킨다).

### 산출물 — [`docs/네이버_스마트플레이스_등록.md`](docs/네이버_스마트플레이스_등록.md)

사장님이 등록 화면에 **그대로 복사해 넣는** 시트. 미클레임 플레이스 확인 절차,
업체명·주소·대표번호·영업시간·업종·소개글·키워드, 등록 직후 할 일(URL 회신·사진·
리뷰 동선)까지 담았다. 값은 전부 `lib/site.ts`에서 뽑아 적었다.

**대표번호는 `053-525-0424`**여야 한다는 점을 강조했다 — CTA 버튼이 전부 모바일로
걸려 있어 모바일을 대표번호로 넣기 쉬운데, 그러면 구조화 데이터의 `telephone`과
어긋난다.

### 검증

`tsc` · `lint` · `build` 통과. First Load JS 103 kB **변동 없음**.
빌드 산출 HTML에서 JSON-LD를 파싱해 확인 — 영업시간 2구간(월~금·토), `areaServed`
7개, `hasMap` 존재, `aggregateRating` 없음.
...
### 마무리 — 비밀번호 최소 길이 6 → 8
재설정 화면은 8자를 요구하는데 서버(`password_min_length`)는 6자였다. 화면을 거치면
문제가 없지만 Supabase 대시보드에서 직접 바꾸면 6자가 통과해, 기준을 8자로 맞췄다.
기존 비밀번호는 그대로 쓸 수 있고 다음에 바꿀 때부터 적용된다.
